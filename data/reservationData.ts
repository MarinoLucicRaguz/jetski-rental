...
export const getFirstReservationsByJetskiIds = async (jetski_ids: number[]): Promise<{ [key: number]: ExtendedReservation | null }> => {
  try {
    ...
    jetski_ids.forEach((jetski_id) => {
      const reservation = reservations.find((res) => res.reservation_jetski_list.some((jetski) => jetski.jetski_id === jetski_id));
...
export const getReservationsByDateAsync = async (date: Date): Promise<ExtendedReservation[] | null> => {
  try {
