...
const JetskiReservationCard: React.FC<JetskiReservationCardProps> = ({ jetski, reservations, locations }) => {
  ...
  //       (jetskiItem) =>
  //         jetskiItem.jetski_id !== jetski.jetski_id &&
  //         jetskiItem.jetski_status !== 'AVAILABLE'
  //     );
  ...
  //       (reservation.endTime <= now && !reservation.isCurrentlyRunning)
  //     ) {
  //       return {
  ...
  //     }
  //   };
  //importati iz dashboardreservationcolor
  ...
  const jetskiReservations = reservations
    .filter((reservation) => reservation.reservation_jetski_list.some((jetskiItem) => jetskiItem.jetski_id === jetski.jetski_id))
      ...
      {jetskiReservations.length > 0 ? (
        <div className="space-y-4">
          {jetskiReservations.map((reservation) => {
            // const { backgroundColor, statusText } = getBackgroundColor(
            //   reservation,
            //   jetski
            // );
            return (
              <div key={reservation.reservation_id} className="border rounded-lg shadow-sm flex flex-col p-4 space-y-2">
                <div className="flex flex-col space-y-1">
                  ...
                  <p>
                    <span className="font-semibold">Location:</span> {getLocationName(reservation.reservation_location_id)}
                  </p>
                  <p>
                ...
                <div className={`rounded-lg shadow-sm flex flex-col p-2 ${1}`}>
