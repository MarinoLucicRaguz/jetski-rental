  ...
  useEffect(() => {
    if (selectedLocation) {
      const fetchJetskis = async () => {
        try {
          const jetskiData = await listJetskisByLocation(selectedLocation.location_id);
              ...
              {jetskis.length > 0 ? (
                jetskis.map((jetski) => (
                  <div key={jetski.jetski_id} className="m-4">
                    <JetskiReservationCard jetski={jetski} reservations={reservations} locations={locations}/>
                  </div>
                ))
              ) : (
