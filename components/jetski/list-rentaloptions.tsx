        ...
        <div className="p-10 bg-white rounded-sm">
            ...
            <table className="w-full text-sm text-left text-gray-500 divide-y">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        <th className="px-6 py-3 cursor-pointer" onClick={() => handleSort("rentaloption_description")}>Type of Rental</th>
                ...
                <tbody className="bg-white divide-y">
                    {sortedRentalOptions?.map((rental) => (
                                ...
                                {user?.role === "ADMIN" && (
                                    <td className="px-4 py-2">
                                        ...
                                        {rental.isAvailable === true ? (
                                            <Button className="ml-2 w-60" variant="destructive" onClick={() => handleDeleteClick(rental.rentaloption_id)}>Disallow rental option</Button>
                                        ) : (
                                            <Button className="ml-2 w-60" variant="constructive" onClick={() => handleDeleteClick(rental.rentaloption_id)}>Allow rental option</Button>
                                        )}
                                    </td>
                                )}
                            </tr>
