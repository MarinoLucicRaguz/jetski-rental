...
export const calculateAvailability = async (
  ...
  reservations.forEach((reservation) => {
    ...
    const reservationEndWithBuffer = new Date(
      reservationEndTime.getTime() + BUFFER_MINUTES * 60 * 1000
    );

    slots.forEach((slot, index) => {
      if (
        reservationStartTime <
          new Date(slot.end.getTime() + BUFFER_MINUTES * 60 * 1000) &&
        reservationEndWithBuffer > slot.start
      ) {
        const locationFiltered =
          !location ||
          reservation.reservation_jetski_list.some(
            (jetski) => jetski.jetski_location_id === location
          );
        const reservedJetskis = reservation.reservation_jetski_list.length;

        if (
          locationFiltered &&
          numberOfAvailableVehicles - reservedJetskis < jetskiCount
        ) {
